...
- `cloud_init_additional_values` (map of string) - Additional Cloud-Init values to pass through to Proxmox as key/value pairs.
  ...
  If both dedicated fields (`nameserver`, `searchdomain`, `ipconfig`) and matching keys are set here,
  `cloud_init_additional_values` takes precedence.
